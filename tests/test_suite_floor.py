...
`ci.yml`'s original guard was `grep -qE '[1-9][0-9]* passed'`, which is
satisfied by "1 passed". These tests exist because that guard's defect
was invisible to every other check in the pipeline: the exit code was 0,
the zero-skip grep found no skips, and coverage is a RATIO, so deleting
a test together with the code it covered can raise it.

The tests below drive `scripts/check-suite-floor.sh` as a subprocess and
read its exit code, rather than reimplementing its parsing here. A test
that reimplements the logic it is checking agrees with itself by
construction.
"""
...
def test_the_script_exists_and_is_the_one_ci_runs() -> None:
    ...
    Every other test here would pass vacuously if the path were wrong:
    bash exits 127, the assertions on non-zero exit codes still hold,
    and the suite stays green while checking nothing. A search at a path
    that does not exist returns a clean empty, which is
...
def test_the_original_guards_blind_spot_is_now_caught() -> None:
    ...
    This is the exact scenario L6 named: a suite that has lost almost
    every test still prints a passing summary, and the old guard could
    not tell it from a healthy run.
    """
...
def test_output_with_no_summary_line_is_a_failure_not_a_pass() -> None:
    ...
    An empty or truncated capture must not read as success. The failure
    mode this forbids is a guard that treats "I could not find a count"
    as "the count was fine" - which is how an except-branch that fails
    closed still fails OPEN on empty input.
    """
...
def test_a_tests_own_stdout_cannot_spoof_the_count() -> None:
    ...
    A test that prints something resembling a summary line - this suite
    has tests that assert on pytest output - would otherwise be read as
    the run's own result, and it appears EARLIER than the real summary.
    """
...
def test_a_floor_that_is_not_a_count_is_a_usage_error(bad_floor: str) -> None:
    ...
    Collapsing these would make a typo in ci.yml look like a real
    regression, and the person debugging it would go looking for deleted
