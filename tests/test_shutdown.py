...
DESIGN.md:1289-1295. Three of the design's stated verification gaps close
...
**Both transports, because they fail differently** (DESIGN.md:1294-1295).
...
def test_sigterm_runs_lifespan_teardown(tmp_path: pathlib.Path, transport: str) -> None:
    ...
    # DESIGN.md:1291-1293: resolve the INTERPRETER via /proc/<pid>/cmdline
...
def test_only_stdio_exercises_the_forced_exit(tmp_path: pathlib.Path) -> None:
    ...
    DESIGN.md:959-961 records that on stdio a non-daemon AnyIO worker thread
...
def test_the_shipped_entry_point_is_what_the_case_exercises() -> None:
    ...
    DESIGN.md:964-981 is explicit that the mitigation this replaced was also
    ...
    # DESIGN.md:974-978 places it there so teardown completes first.
...
def test_the_handler_does_not_read_ambient_state() -> None:
    """DESIGN.md:947-953: `getsignal(SIGINT)` is the defect, not the fix.
