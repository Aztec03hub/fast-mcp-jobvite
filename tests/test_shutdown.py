...
DESIGN.md:1421-1427. Three of the design's stated verification gaps
...
(DESIGN.md:1111-1113). The HTTP arm passes on teardown alone; **only the
...
def test_sigterm_runs_lifespan_teardown(tmp_path: pathlib.Path, transport: str) -> None:
    ...
    try:
        ...
        # DESIGN.md:1110-1111: resolve the INTERPRETER via
...
def test_only_stdio_exercises_the_forced_exit(tmp_path: pathlib.Path) -> None:
    ...
    DESIGN.md:1054-1056 records that on stdio a non-daemon AnyIO worker
...
def test_the_shipped_entry_point_is_what_the_case_exercises() -> None:
    ...
    DESIGN.md:1099-1107 is explicit that the mitigation this one
    ...
    # only: DESIGN.md:1069-1083 places it there so teardown completes
...
def test_the_handler_does_not_read_ambient_state() -> None:
    """DESIGN.md:1042-1048: `getsignal(SIGINT)` is the defect.
...
def test_a_crashing_mcp_run_exits_70_read_from_the_process(
    ...
    """ADR-0018 and DESIGN.md:1088-1096, on the PROCESS's status.
    ...
    **The structural assertion above is not a discharge of this.** It
    ...
    the cheapest one, and the one DESIGN.md:1094-1096 names - and reads
