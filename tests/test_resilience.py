...
# ======================================================================
# THE SCAN'S BOUNDS - R5-H2 and ADR-0024 (Accepted, 2026-08-29).
#
...
# than argued.** `scripts/probe-scan-bounds.py`, re-run on `5eb64b0`
# against a client that HAS the budget, had to abort ALL FOUR of its
# arms at 2,000 requests - including the one with a two-SECOND budget,
# because the budget bounds WALL CLOCK and a `MockTransport` answers
# thousands of requests inside it. ADR-0024 says the budget is "a
# mitigation, not a fix"; the measurement is stronger than that, because
# the budget did not fire at all.
#
...
async def test_a_fully_duplicate_page_is_still_not_a_short_page() -> None:
    ...
    # The other half of the outcome, and it is a WEAK assertion by
    # measurement rather than by intention: A20 and A22 both survive
    # this case, and the request count above already kills the
    # break-fires-always mutation. It states that this scan ended on
    # its SHORT PAGE and not on a bound, which the counts alone do not
    # distinguish from a scan that paged correctly and then declared
    # itself truncated anyway.
    assert result.incomplete is False
