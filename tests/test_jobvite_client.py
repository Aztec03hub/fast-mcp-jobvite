...
searched the module's own file for a string that the module's docstring quoted -
it was asserting that the documentation existed. Where this suite has to inspect
the source at all it walks the AST (`test_no_module_grep_...` below is the one
...
class _Secret:
    ...
    Mirrors `pydantic.SecretStr`'s one relevant method. `fakes must mirror real
    types`: the production type U1 will pass in is `SecretStr`, and this
    satisfies the same `Protocol` the client declares, so a client change that
    broke `SecretStr` compatibility would break this too.
    """
...
def read_fixture(name: str) -> bytes:
    ...
    Bytes, not text: "byte-exact" is the claim, and `read_text` would normalise
    a line ending on some platforms and quietly make the claim false.
    """
...
def client(
    ...
    *,
    ...
    """Build a client whose transport is `httpx2.MockTransport` (ADR-0007)."""
...
# ===========================================================================
# §8 #1 - THE 200-WITH-401-BODY TRAP. C5-S1, the only Critical on the client.
# ===========================================================================


def test_the_recorded_200_with_401_body_fixture_is_byte_exact() -> None:
    """Pin the ground truth itself before asserting behaviour against it.

    `error_auth_200_body401.json` is a [RECORDED] capture. If someone
    regenerates or "tidies" it, the case below would still pass against the
    tidied bytes while no longer testing the transport Jobvite actually emits.
    """
...
async def test_C5_S1_an_http_200_carrying_a_401_body_is_NOT_a_success() -> None:
    ...
    key and reports zero candidates for a REFUSED credential.
    """
    ...
    # credential that failed is the one THIS SERVER holds.
...
async def test_positive_control_a_200_with_no_status_block_at_all_SUCCEEDS() -> None:
    ...
    Whether a success body carries a `status` block is unknown (checklist row
    §13.1), so BOTH shapes must pass. A client that required the block would
    turn every real success into an error the day a credential lands.
    """
...
# ===========================================================================
# The four remaining RECORDED fixtures, byte-exact. With #1 above that is five,
# and the recorded tier is exactly five files - all of them.
# ===========================================================================
...
def test_the_recorded_tier_is_exactly_these_five_files() -> None:
    ...
    `IMPLEMENTATION-PLAN.md:786-789` claims the recorded tier is five files and
    that the list is exhaustive. A test that asserted only the five it knew
    about would pass if a sixth were added and never noticed.
    """
    recorded = {
...
def test_recorded_error_fixtures_are_byte_exact() -> None:
    ...
    # NOTE the absence of a trailing newline, where the other four have one.
    # This assertion was written with a `\n` and the byte-exact check caught it,
    # which is the whole argument for pinning bytes rather than parsed content:
    # every one of these fixtures round-trips identically through `json.loads`.
...
def test_the_two_auth_fixtures_are_byte_identical_only_http_status_differs() -> None:
    ...
    `error_auth_401.json` are the same bytes. A reader who assumed the fixture
    NAME carried the 200 would write a test that passes while loading the wrong
    file, and it would still be green.
    """
...
async def test_a_json_envelope_401_on_an_http_401_fails() -> None:
    """`error_auth_401.json` - the conventional case, both arms agreeing."""
...
async def test_plain_text_with_no_content_type_fails_loudly() -> None:
    ...
    `Content-Type` header at all**, which is what rules content-type sniffing
    out as the dispatch. The MockTransport response here sends none either, so
    the test exercises that condition rather than describing it.
    """
...
async def test_a_route_level_404_is_not_reported_as_a_record_not_found() -> None:
    ...
    `404 "Invalid URL Cannot find API."` means the ROUTE does not exist. The
    record-level not-found shape is unknown (`JOBVITE-CONTRACT.md` §3.4,
    checklist row §13.4). Mapping this to `RESOURCE_NOT_FOUND` would tell the
    caller their candidate id was wrong when the truth is that we called a URL
    that is not there - a bug in this server reported as the caller's mistake.
    """
...
async def test_a_malformed_body_fails_loudly_rather_than_degrading(name: str) -> None:
    ...
    different road: an `except ValueError: return {}` turns an undecodable body
    into "no candidates found".
    """
...
async def test_a_malformed_body_on_a_200_reports_no_upstream_status() -> None:
    ...
    Claiming `upstream_status == 200` would say the failure WAS the 200, which
    inverts the field's meaning.
    """
...
async def test_valid_json_that_is_not_an_object_fails() -> None:
    """A bare list or `null` decodes fine and is still not a Jobvite body."""
...
async def test_arm_2_an_http_500_with_a_passing_envelope_still_fails() -> None:
    ...
    Without this case, deleting `if http_status >= 400` from the client leaves
    every other test in this file green: the recorded fixtures all carry a
    failing `status.code`, so arm 1 catches them all. This body is synthetic.
    """
...
async def test_arm_1_fires_even_when_the_http_status_is_a_success() -> None:
    """Symmetric to the test above: the envelope arm where HTTP cannot help.

    Together these two are what make "both, every call" a tested claim rather
...
async def test_an_xml_entity_bomb_is_REFUSED_rather_than_expanded() -> None:
    ...
    A billion-laughs document expands to gigabytes under the stdlib parser. The
    assertion that matters is that we get a typed error quickly instead of an
    expanded document - and note this arrives on an HTTP 200, so nothing about
    the transport warned us first.
    """
    bomb = (
        b'<?xml version="1.0"?>'
    ...
    async with client(responder(200, bomb)) as c:
        with pytest.raises(JobviteUpstreamError) as caught:
            await c.request("GET", "/candidate")
    # The expansion never happened: nothing that long reached the message.
...
async def test_positive_control_defusedxml_still_parses_an_ordinary_document() -> None:
    ...
    Without this, a `_raise_from_markup` that ignored its parse entirely would
    pass the bomb case above.
    """
...
async def test_the_jobfeed_route_refuses_without_a_company_id() -> None:
    """A missing `companyId` would come back as the same 401 as a bad secret."""
...
def test_the_client_and_the_redactor_name_the_SAME_two_headers() -> None:
    """Pins the two lists together (`fix one instance, check its siblings`).

    Renaming a header here while `utils/redaction.py` keeps the old name leaves
    the redactor watching a header that no longer exists - and it fails OPEN,
    silently, because a redactor that matches nothing still returns a mapping.
    """
...
async def test_an_error_body_quoting_a_credential_is_redacted_before_detail() -> None:
    """The body is attacker-influenced text that reaches an exception message."""
...
async def test_an_enormous_error_body_is_truncated_before_reaching_detail() -> None:
    """A body we do not control must not become an unbounded log line."""
...
async def test_no_cookie_jar_is_carried_between_requests() -> None:
    """The `AWSALBAPP-*` values are the literal `_remove_`; there is no session.

    **This is not httpx2's default.** A bare `AsyncClient` stores these and
    sends them back, which the positive control below measures directly rather
...
async def test_positive_control_httpx2_DOES_carry_cookies_by_default() -> None:
    ...
    If httpx2 kept no jar of its own, the assertion above would pass against a
    client that did nothing, and "no cookie jar" would be an untested claim
    about someone else's default. This asserts the default IS to carry them, so
    the clearing above is doing real work. It is the control on the control.
    """
    sent: list[str | None] = []
...
async def test_the_jar_is_cleared_even_when_the_call_RAISED() -> None:
    """Clearing lives in a `finally`, so a failed call leaves nothing behind."""
...
# ===========================================================================
# Structural guards. The one place this suite inspects source - via the AST.
# ===========================================================================
...
def test_no_third_party_mocking_library_is_imported_anywhere_in_the_suite() -> None:
    ...
    existed. `ast` sees imports and nothing else.
    """
