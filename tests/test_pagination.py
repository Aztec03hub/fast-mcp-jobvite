"""U6 - paging (DESIGN.md:471-526, `IMPLEMENTATION-PLAN.md` U6).
...
1. **De-duplication is a defence against OVER-reading only**
   (DESIGN.md:504-507). `test_de_duplication_cannot_recover_a_record...`
...
2. **The completeness check has TWO arms** (DESIGN.md:508-516). It
   ...
   worked example and DESIGN.md:513 says alarming on the default path
...
`start` is 1-based is a VENDOR CLAIM (DESIGN.md:490) and not an
        ...
        *,
        ...
        Args:
            ...
            base: The server's own base, and the two values are the two
                hypotheses DESIGN.md:499-501 says the evidence cannot
...
async def test_every_scan_starts_at_zero_on_the_wire() -> None:
    """DESIGN.md:494. Read off the transport, not off the source.
...
async def test_start_zero_holds_on_the_jobfeed_route_too() -> None:
    """The base is per RESOURCE (DESIGN.md:517-519), the start is not.
...
async def test_a_zero_based_server_returns_record_zero() -> None:
    ...
    The paired direction of DESIGN.md:502-503: *starting at 1 is the
...
async def test_an_overlapping_page_drops_duplicates() -> None:
    """DESIGN.md:504-505, the behaviour half.
...
async def test_de_duplication_cannot_recover_a_never_returned_record() -> None:
    """DESIGN.md:504-507, **THE LIMITATION, WHICH IS THE POINT**.
...
async def test_records_without_an_id_are_kept_not_collapsed() -> None:
    ...
    deleting real records, which is the failure DESIGN.md:504-507 is
...
async def test_the_scan_terminates_on_a_short_page() -> None:
    """DESIGN.md:525. `len(items) < count` is the only stop rule."""
    server = Recorder(records=records(7))
...
async def test_a_total_that_understates_does_not_end_the_loop_early() -> None:
    """DESIGN.md:526. `total` is reported and never a loop condition.
...
async def test_completeness_does_not_fire_on_a_capped_call() -> None:
    ...
    DESIGN.md:512-516: a capped call is a mismatch **by design** -
...
def test_the_transport_caps_are_the_designs_figures() -> None:
    """DESIGN.md:473. 500 on v2, 1000 on `/v1/jobFeed`.
...
def test_the_result_cap_is_the_min_of_the_two_halves() -> None:
    """DESIGN.md:473-475, the `min()` this unit owns.
...
async def test_the_jobfeed_route_uses_its_own_transport_cap() -> None:
    """1000 on `/v1/jobFeed`, per resource (DESIGN.md:473).
...
async def test_a_clamped_page_still_returns_no_more_than_the_limit() -> None:
    ...
    **This is the one shape where the in-loop break cannot hold the
    ...
    exists for (DESIGN.md:499-501). A page that is FULL on the wire but
...
def test_the_scan_start_defaults_to_zero_for_every_resource() -> None:
    """DESIGN.md:494. No resource ships a declared base of 1.
...
async def test_an_override_is_per_resource_and_not_global() -> None:
    """DESIGN.md:517-519 - the base is per-resource, not global.
...
async def test_a_scan_is_whole_under_both_surviving_hypotheses(base: int) -> None:
    ...
    DESIGN.md:499-501 says the evidence cannot separate "0-based" from
...
async def test_an_over_read_is_not_logged_as_an_under_read() -> None:
    ...
    says **fewer** - which is what DESIGN.md:508-516 describes and
    all it contemplates.
    ...
    OVER-read announced as an under-read - and DESIGN.md:513 is
