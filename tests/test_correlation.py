"""U2: `request_id_var` (DESIGN.md:661-672).
...
def test_the_id_does_not_leak_past_the_scope() -> None:
    """DESIGN.md:665-666 - a reused task must not inherit an id.
...
def test_the_reset_is_lexically_in_a_finally() -> None:
    ...
    DESIGN.md:665-666 asks for a `finally` specifically, so the
...
async def test_concurrent_invocations_never_read_each_others_id() -> None:
    """DESIGN.md:668-672 - what a module global would cause.
...
def test_correlation_declares_exactly_one_contextvar() -> None:
    """DESIGN.md:664 - "a single ContextVar".
