"""U2: the error contract (DESIGN.md:485-525, IMPLEMENTATION-PLAN.md:447-470).

The table below is the design's registry table (DESIGN.md:507-515) restated as
...
def test_a_jobvite_401_is_a_502_and_not_a_401() -> None:
    """DESIGN.md:500-503: returning 401 blames a credential the caller cannot hold."""
...
def test_validation_is_422_and_not_400() -> None:
    """DESIGN.md:519: validation is 422 per the registry, not 400."""
...
def test_every_problem_carries_all_seven_required_members() -> None:
    """error-contract.md:66 elevates these seven to required; DESIGN.md:489-490."""
...
def test_jobvites_own_status_and_message_are_in_detail_and_not_discarded() -> None:
    ...
    # Preserved on the exception too, for the audit event (DESIGN.md:517-518).
...
def test_a_jobvite_failure_with_no_status_still_keeps_its_message() -> None:
    """The plain-text and Tomcat-HTML encodings carry no status (DESIGN.md:339-341)."""
    problem = errors.problem_from_exception(
...
def test_an_unmapped_exception_does_not_leak_its_message_to_the_caller() -> None:
    """An arbitrary exception's str() can carry a URL or a credential fragment."""
    problem = errors.problem_from_exception(
...
def test_a_problem_object_is_returned_never_raised() -> None:
    """DESIGN.md:521-525 - being returned is what no configuration can distort.
    ...
    for condition, factory, expected_type, _status in REGISTRY_CASES:
        # If the call raised instead of returning, this never reaches the assert.
        problem = errors.problem_from_exception(factory(), RID)
...
def test_no_problem_construction_function_raises_a_problem_object() -> None:
    ...
    The behavioural arm above cannot see a `raise` on a path it does not take.
    The only permitted `raise` in this module is the ValueError guarding
    extension-member shadowing, which is a programming error at the call site.
    """
...
def test_extension_members_survive_alongside_the_seven() -> None:
    """DESIGN.md:352 attaches a retry_after hint to the 503."""
...
# `error-contract.md:96-108`, pinned here as data. It is pinned rather than read
# from the standards checkout because CI does not check that repository out - a
# test that reads it would hard-fail in CI, and one that tolerates its absence
# would be a wrong zero. Drift between this table and the standard is caught by
# the project's citation audit, not by this suite; that limit is stated in
# docs/worklogs/U2-REPORT.md.
...
def _defined_kinds() -> dict[str, tuple[str, int]]:
    """Every ProblemKind errors.py defines, except the about:blank fallback."""
    return {
...
def test_the_registry_constants_match_the_standards_table_verbatim() -> None:
    """error-contract.md:96-108 is the source; a drifted title is a contract break."""
    assert _defined_kinds() == REGISTRY_TABLE


def test_no_type_uri_is_minted_locally() -> None:
    """DESIGN.md:504-505: a published type URI is a promise owed forever.

    The count is asserted first. Without it, deleting every registry constant
    makes this loop iterate zero times and pass - which is how a repo-wide
    "nothing bad exists" assertion goes vacuous.
    """
...
def _scan_for_envelope(roots: list[pathlib.Path]) -> list[str]:
    """Return `path:line` for every `success: true/false` envelope under `roots`.

    A `git grep` was the first form of this and it was wrong: it searches
    **tracked** files only, so a brand-new untracked module returns a clean,
    self-explaining zero.
    """
    hits: list[str] = []
    scanned = 0
    for root in roots:
...
def test_no_success_true_false_envelope_exists_anywhere_in_the_repository() -> None:
    ...
    **This assertion is near-vacuous today and U2-REPORT.md says so**: `src/`
    holds four modules, so it passes over almost nothing. It must be re-asserted
    once tools exist. The two controls above are what keep it from being a wrong
    zero.
    """
