...
timeout 900 uv run --frozen pytest $SUITE -q -p no:cacheprovider >/tmp/u3-base.txt 2>&1
baseline_rc=$?
...
PY
  ...
  timeout 900 uv run --frozen pytest $SUITE -q -p no:cacheprovider -rf >/tmp/u3-mut.txt 2>&1
  local rc=$?
  if [ "$rc" -eq 124 ]; then
