...
# ===========================================================================
# THE SCAN'S BOUNDS (R5-H2, ADR-0024 Proposed). `scan()` had none at all.
# ===========================================================================
...
mutate "M23 the zero-progress break can never fire" \
  "$CLIENT" \
...
mutate "M24 progress is sampled after the page, so the break fires always" \
  "$CLIENT" \
...
mutate "M25 a stalled scan reports itself as complete" \
  "$CLIENT" \
  "$SUITE::test_a_server_that_ignores_start_is_bounded_after_one_wasted_page" \
  '            or stalled
            or ceiling_hit' \
...
# proposal and what this unit amended. At 50 records per page it admits
...
mutate "M26 the ceiling counts pages instead of records" \
  "$CLIENT" \
...
mutate "M27 the record ceiling can never fire" \
  "$CLIENT" \
