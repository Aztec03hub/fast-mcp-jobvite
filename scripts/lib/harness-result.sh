...
#
# WHY NO `fired=` FIELD. The first draft of the grammar had one. Across this
# family the "fired" tally has FOUR incompatible meanings - `N/M controls fired.`,
# `RESULT: N killed, M not killed`, `ROWS: N   ANCHORS APPLIED: M`, and for the
# amputation harnesses an INVERTED pass condition where survivors are the OUTPUT
# rather than a failure (which is why `ci-harness-gate.sh` has an `--amputation`
# flag at all). `ci-harness-gate.sh` states the reason itself, about the very
# phrases such a field would absorb: "each is printed beside a different
# diagnosis, and collapsing them would send the next reader to the wrong place."
# A field carrying four meanings lies more loudly than an absent one, and
# `status` below already carries the verdict - from the real exit code, not from
# a counter a refactor could leave behind.
#
...
harness_result_emit() {
  ...
  printf 'HARNESS-RESULT name=%s rows=%s floor=%s status=%s\n' \
    "$HR_NAME" "$HR_ROWS" "$HR_FLOOR" "$status"
