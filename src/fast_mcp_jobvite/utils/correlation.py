...
**Why a ContextVar and not a module global** (DESIGN.md:608-612): `asyncio` runs
...
**Why it exists at all** (DESIGN.md:601-604, B40): the retry and circuit-breaker
...
def request_id_scope(request_id: str) -> Iterator[str]:
    ...
    DESIGN.md:604-606 requires that `audit.py` set the var in the same statement
