"""The per-invocation correlation ContextVar (DESIGN.md:586-597).
...
**Why a ContextVar and not a module global** (DESIGN.md:593-597): `asyncio` runs
...
**Why it exists at all** (DESIGN.md:586-589, B40): the retry and circuit-breaker
...
def request_id_scope(request_id: str) -> Iterator[str]:
    ...
    DESIGN.md:589-591 requires that `audit.py` set the var in the same statement
