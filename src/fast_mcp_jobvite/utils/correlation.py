"""The per-invocation correlation ContextVar (DESIGN.md:661-672).

**The name `request_id_var` is not a choice.**
`ai/tool-calling.md:173-175` mandates the canonical triple verbatim -
HTTP header `X-Request-ID`, log field `request_id`, ContextVar
`request_id_var` - so using it discharges the clause rather than merely
resembling it. Renaming this variable breaks compliance, not only
imports.

**Why a ContextVar and not a module global** (DESIGN.md:668-672):
...
every line still carries a well-formed UUID. That is the failure this
mechanism exists to prevent, which is why the tests assert under
concurrency rather than on a single call.

**Why it exists at all** (DESIGN.md:661-664, B40): the retry and
...
def request_id_scope(request_id: str) -> Iterator[str]:
    ...
    DESIGN.md:664-666 requires that `audit.py` set the var in the same
    ...
    each call site: a leak test that only exercises a `try/finally`
    written inside the test proves nothing about what the server does.

    `ContextVar.reset(token)` is used rather than `set(None)`, so a
    nested scope restores the enclosing id instead of erasing it.

    Args:
        request_id: The UUIDv4 minted for this invocation.

    Yields:
        The same `request_id`, so the caller can bind and use it in one
        statement.
    """
