...
invariant this module is built around:
    ...
    **and** the HTTP status is below 400. Both, every call.

The trap it defends against is recorded, not hypothetical
(`JOBVITE-CONTRACT.md` §3.1): `api.jobvite.com` answers a rejected credential
with **HTTP 200** and a body of
...
credential that was refused**. A wrong zero that explains itself is the hardest
kind to notice, which is why this is C5-S1, the only Critical on the client.

**The two arms are written as two independent statements on purpose.** Either
one alone passes a plausible-looking test suite:

* Drop the envelope arm and the recorded 200-with-401-body fixture is reported
  as a success.
...
from ..utils.redaction import redact_text, redact_url

# ---------------------------------------------------------------------------
# Transport constants. `JOBVITE-CONTRACT.md` §2 records both base URLs as
# [RECORDED]; note the v1 base is `/v1`, NOT `/api/v1`.
# ---------------------------------------------------------------------------
...
# noqa on the next line: S105 flags the NAME of a header, not a credential. The
# value never appears in this file; it arrives as a SecretValue at construction.
...
#: How much of an undecodable body may appear in an exception `detail`. Jobvite's
#: Tomcat HTML page is small, but nothing guarantees the next one is, and this
#: string reaches a log line. Truncation is a bound on the blast radius, not a
...
class SecretValue(Protocol):
    ...
    **Structural rather than nominal, and that is deliberate.** DESIGN.md:323-324
    ...
    `fastmcp`, and U4's dependency slot is granted for `httpx2` and `defusedxml`
    and nothing else, so importing it here would either add an undeclared direct
    import or spend a slot that was not granted.

    A `Protocol` satisfies the design's actual requirement - the value is opaque
    until something deliberately unwraps it - and pydantic's `SecretStr`
    satisfies this Protocol structurally, so U1's configuration can hand one
    straight in with no adapter. See U4-IMPL-REPORT.md.
    """
...
def evaluate_response(http_status: int, body: bytes) -> dict[str, Any]:
    ...
    **The invariant (DESIGN.md:332-333): a response is successful only if the
    ...
    Args:
        http_status: The HTTP status line's code. **Not authoritative on its
            own** - see the module docstring and `JOBVITE-CONTRACT.md` §3.1.
        body: The raw response bytes. Raw rather than a parsed object, because
            the decision of whether this is even JSON is part of the rule.

    Returns:
        The decoded JSON body, when and only when both arms of the invariant
        hold.

    Raises:
        JobviteUpstreamError: On either arm of the invariant failing, and on a
            body that cannot be decoded as a JSON object at all - which includes
            the plain-text, HTML and XML encodings, all of which are errors.
    """
    payload = _decode_json_object(http_status, body)

    # ARM 1 - the body's own status envelope. This is the arm the recorded
    # 200-with-401-body fixture exercises, and the reason the HTTP status is not
    # consulted first: at that point the HTTP status is 200 and says nothing.
    ...
    # ARM 2 - the HTTP status, applied INDEPENDENTLY rather than as an `elif`.
    # A body with no `status` block at all, or with a code under 400, arriving
    # on a 5xx must still fail. `JOBVITE-CONTRACT.md` §3.2 records that a status
    # block with a code under 400 has never been observed, so this arm is what
    # covers the shape we cannot rule out.
...
def _envelope_status_code(payload: Mapping[str, Any]) -> int | None:
    ...
    `None` and a code under 400 are different answers and are kept apart:
    `JOBVITE-CONTRACT.md` §3.2 records that no success body has ever been
    observed, so whether a success carries a `status` block at all is unknown
    (checklist row §13.1). Both are tolerated, and neither is treated as an
    ...
    # `bool` is an `int` in Python, and `{"code": true}` is not a status code.
...
def _decode_json_object(http_status: int, body: bytes) -> dict[str, Any]:
    ...
    **This is the "cannot assume JSON, cannot dispatch on content type" half**
    ...
    Every non-JSON encoding is an ERROR. That is not an assumption: v2 speaks
    JSON on success, so plain text, HTML and XML are all failure shapes, and the
    two synthetic malformed fixtures must fail loudly here rather than degrade
    to an empty result.
    """
    ...
    try:
        ...
        # Plain text with no Content-Type - the recorded v1 `jobFeed` 401 - and
        # the synthetic malformed bodies. FAIL LOUDLY: returning `{}` here is
        # precisely the "wrong zero" this module exists to prevent.
        raise JobviteUpstreamError(
            _upstream_status_or_none(http_status), _excerpt(text)
        ) from None

    if not isinstance(payload, dict):
        # Valid JSON that is not an object: `null`, a bare list, a number. No
        # Jobvite route is documented to return one, and `payload["status"]`
        # would raise a TypeError several frames away from the cause.
        raise JobviteUpstreamError(
...
def _raise_from_markup(http_status: int, text: str) -> NoReturn:
    ...
    **HR-XML is a hardened fallback, not a handled case** (DESIGN.md:337-340).
    ...
    expansion, and that whatever comes out is reported as a failure.

    `defusedxml` raising is a SUCCESSFUL outcome for us: it means the document
    carried a DTD, an external entity or an entity bomb, and we declined to
    expand it. Either way the caller gets a `JobviteUpstreamError`; the only
    difference is how much detail we can honestly put in it.
    """
    detail = _excerpt(text)
    try:
        root = defused_fromstring(text)
    except (DefusedXmlException, SyntaxError):
        # Not well-formed XML (the Tomcat HTML page), or defused_etree refused
        # it (a DTD or an entity bomb). Both are errors and neither is parsed
        # further. The body excerpt is redacted and truncated, never trusted.
        raise JobviteUpstreamError(
            _upstream_status_or_none(http_status), detail
        ) from None

    # Well-formed XML. Pull the HR-XML `<Error code="N">` shape if it is there;
    # anything else still ends as an error, just with a vaguer detail.
...
def _upstream_status_or_none(http_status: int) -> int | None:
    ...
    `JobviteUpstreamError(None, ...)` renders as "Jobvite returned status none",
    which is the honest thing to say about a body that failed to decode on an
    HTTP 200: Jobvite reported no failing status anywhere, and claiming it
    returned 200 as though 200 were the error would invert the meaning of the
    field.
    """
...
class JobviteClient:
    ...
    **`request` is the single method that issues one call, applies the invariant
    and returns a decoded body or raises the typed error**
    (`IMPLEMENTATION-PLAN.md:773-777`). Paging around it is U6's; U4 owns the
    one-call path. Without this method U4 would be an auth-and-error module with
    no caller.

    ADR-0007 chose `httpx2`: `fastmcp 4.0.0b4` does not install `httpx` at all,
    and `httpx2` ships `MockTransport` in the box, which is what lets the
    ...
    async def __aenter__(self) -> Self:
        """Enter the client's context, so the connection pool is closed."""
        return self
    ...
    def jobfeed_params(self) -> dict[str, str]:
        ...
        Raises:
            JobviteUpstreamError: If no `company_id` was configured. Raised
                rather than sending the call without it, because Jobvite would
                answer a missing `companyId` with the same 401 body as a bad
                credential and the operator would go looking for the wrong fault.
        """
    ...
    async def request(
        ...
        *,
        ...
        """Issue ONE Jobvite call, apply the invariant, return the decoded body.

        **The single request entry point.** One call, no paging - paging around
        this method is U6's (`IMPLEMENTATION-PLAN.md:775`).
        ...
        Args:
            ...
            jobfeed: Select the v1 `jobFeed` route and its query-parameter
                authentication. **A `bool` rather than a free-form base URL**:
                the sensitive-URL path has to be one enumerated branch that a
                test can point at, not a value a caller can pass in.
        ...
        try:
            ...
            # NO COOKIE JAR (`JOBVITE-CONTRACT.md` §2.3). Jobvite sets four
            # `AWSALBAPP-*` cookies whose values are all the literal string
            # `_remove_`, and the API is credential-authenticated per request:
            # there is no session to carry.
            #
            # **This is not httpx2's default and was measured, not assumed.** A
            # bare `AsyncClient` stores those cookies and sends them back on the
            # next request; the probe that established it is reproduced as a
            # test. Clearing in `finally` rather than after a success means a
            # call that raised cannot leave a jar behind either.
