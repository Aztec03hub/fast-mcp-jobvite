...
| BASH-1 | SUPERSEDED | `docs/adr/0023-harnesses-drop-e-from-strict-mode.md` | `the clause admits no exception` | `devops/bash.md:36-41` | `bash.md:36-41` mandates `set -euo pipefail` and admits no exception; all 15 `scripts/*.sh` run `set -uo pipefail`. ADR-0023 records this as a DEVIATION, not as compliance. The `-e` half is dropped because `out=$(cmd); rc=$?` exits before `rc` is read, so a control cannot tell a fired mutation from an unfired one, and `restore` never runs. Both arms measured by `docs/reviews/probe-set-e-vs-harness.sh`. The shebang half of the same clause is met by 15/15 |
| BASH-2 | ABSENT | - | - | `devops/bash.md:734` | "All scripts MUST pass ShellCheck with zero warnings." **Nothing runs ShellCheck** - the only occurrence of the word in the tree is a `# shellcheck disable=SC2086` in `ci.yml`, a suppression for a linter that never runs. Measured at `eb4d254` with shellcheck 0.10.0 over all 15 scripts: **0 error, 3 warning, 10 info, 2 style**. At `--severity=warning`, the threshold the clause's own CI form (`:741`) and pre-commit block (`:767`) specify, that is **3 findings in 2 files** - so this row is one small fix away from MET, not a rewrite. Left ABSENT rather than wired, because wiring the hook without those 3 fixes lands a red gate |
