...
| BASH-1 | SUPERSEDED | `docs/adr/0023-harnesses-drop-e-from-strict-mode.md` | `the clause admits no exception` | `devops/bash.md:36-41` | `bash.md:36-41` mandates `set -euo pipefail` and admits no exception; all 20 `scripts/*.sh` run `set -uo pipefail`. ADR-0023 records this as a DEVIATION, not as compliance. The `-e` half is dropped because `out=$(cmd); rc=$?` exits before `rc` is read, so a control cannot tell a fired mutation from an unfired one, and `restore` never runs. Both arms measured by `docs/reviews/probe-set-e-vs-harness.sh`. The shebang half of the same clause is met by 20/20. **The count was 15 and went stale as five harnesses landed** - it is a POPULATION, so COUNT `scripts/*.sh` and count the files whose `grep -c 'set -uo pipefail'` is 1, rather than carrying the number forward. A pipe cannot appear in this cell, which is why the commands are described instead of pasted |
