...
| B75 | CONTRADICTED | `.github/workflows/ci.yml:402` | `# - name: Capability drift diff` | `documentation/agentic-coding-standard.md:173` | **Was three commented-out blocks; U11 enabled the advisory one, so two remain and both are U1's** - the capability-drift diff and the coverage floors. Deliberate and reasoned, still with no ADR. Needs a decision, not a deletion |
...
| B78 | SUPERSEDED | `docs/plans/IMPLEMENTATION-PLAN.md:1307` | `headings matching exactly` | `documentation/readme-standard.md:50` | U13 checks the table against `.env.example` rather than hand-keeping it |
| B79 | ABSENT | - | - | `documentation/readme-standard.md:64` | The 500-line cap is named nowhere, and U13 loads the README heavily |
| B81 | SUPERSEDED | `docs/plans/IMPLEMENTATION-PLAN.md:1352` | `A CI status badge` | `documentation/readme-standard.md:70` | Deferred until CI exists, with the deferral distinguished from an excuse |
| B82 | MET | `.github/workflows/ci.yml:555` | `Relative links resolve` | `documentation/readme-standard.md:69` | Uncommitted when seeded |
