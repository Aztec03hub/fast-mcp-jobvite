...
| B49 | MET | `pyproject.toml:154` | `line-length = 88` | `backend/python.md:35` | Code half only. The 72-character comment/docstring half of `python.md:36` is unenforced - see task B49b |
| B50 | MET | `pyproject.toml:183` | `convention = "google"` | `backend/python.md:97` | Type-hint half is `ANN` at `pyproject.toml:158` |
| B51 | MET | `pyproject.toml:176` | `no datetime.utcnow` | `backend/python.md:227` | The `DTZ` rule family. This row is the reason the file exists: deleting it must now break something that names B51 |
| B52 | MET | `pyproject.toml:170` | `pep8-naming` | `backend/python.md:64-71` | The `N` rule family |
