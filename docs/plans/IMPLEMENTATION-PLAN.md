...
High or Medium findings and an empty must-mitigate table (`DESIGN.md:1843`). **The design being
...
them under a unit that had declined them while `DESIGN.md:1808` names them as a **Critical** row's
...
that used the present tense; `DESIGN.md:1463-1469` now states that **every "CI runs" sentence in
...
§8's required-cases list holds **25 bullets**, spanning `DESIGN.md:1262` through `:1306`. I
...
Plus the blanket rule at `DESIGN.md:1367-1368`: **every refusal-path test is paired with a positive
...
`DESIGN.md:1249-1254`. Three tiers, and the split is load-bearing:

- **Recorded** - byte-exact captures of **real Jobvite error transport** (`DESIGN.md:1250`).
...
**The sentence at `DESIGN.md:1256-1257` goes in the test module's own docstring, verbatim in
...
`DESIGN.md:1227-1247`. CI has zero skips; a skip is a failure. Credential-dependent tests are
...
requires a **real resolve**, while `DESIGN.md:1227` requires the default suite to run with **no
...
**Its first act was standing up CI**, because nothing ran there before it (`DESIGN.md:1463-1469`)
...
(`DESIGN.md:1406-1415`); `uv.lock` committed; `[tool.pytest.ini_options]` with
...
(`DESIGN.md:1232-1233`); **the 80% overall coverage floor**; `.github/workflows/ci.yml`. CI's
...
is now written down: they are **U15** (`DESIGN.md:1624-1634`), for the reasons that unit states.
...
- #3: assert against the committed `.gitignore` (currently covers `.env`, `.env.*`, `*.key`,
  ...
  **Seven** of the fifteen variables carry a value, and two of them - `JOBVITE_MAX_RESULTS=50` and
  ...
  `.env.example` carries **no real value** (`DESIGN.md:1269`), meaning no real credential; draft 2
...
carried 72 as a figure quoted from `DESIGN.md:1406-1410` and listed reproducing it among what this
...
**3.12.3** against the design's `>=3.12` floor (`DESIGN.md:1401`), git 2.43.0, gh 2.45.0. Both
...
validation per `DESIGN.md:938-944`, `JOBVITE_TOOLS` allow-list with an unrecognised name as a
...
diffed the two sets rather than counting them, which is the check `DESIGN.md:1543-1549` asks for and
...
- §8 **#10**: `JOBVITE_MCP_HOST` set to a non-loopback address, no certificates,
  ...
  it.
- §8 **#18**, on **both transports** (`DESIGN.md:1337-1343`): lifespan teardown runs on SIGTERM,
  ...
  teardown entirely, `DESIGN.md:979-981`), resolve the interpreter via `/proc/<pid>/cmdline` rather
...
**Inherited limits, not quietly resolved.** `DESIGN.md:1024-1032` states two: the composed
...
**Unblocked by `9d65cc0` - draft 1 had this unit stalled.** `DESIGN.md:1546-1561` now names both
...
**Neither default is verified, and the plan does not describe them as such.** `DESIGN.md:1581-1582`
...
**And the threat-model rows did not move.** C3-I1 (`DESIGN.md:1743`) and C6-D1 (`:1738`) still read
...
(`DESIGN.md:502-532`). `utils/correlation.py` - a single `ContextVar[str | None]` named
...
- A repository-wide assertion that **no `success: true/false` envelope exists anywhere**
  (`DESIGN.md:497`).
- Problem objects are **returned, never raised** - the property `DESIGN.md:536-538` says makes them
...
`DESIGN.md:709-725`); `utils/redaction.py` **secret redaction only** - the fencing half is U8.

**Depends on.** U2.

**Verified by.**

- §8 **#4** (positive) and **#5** (absence), **as a pair** - `DESIGN.md:1277-1280` requires them
...
- §8 **#2**: a secret never reaches a log record, **including the whole `jobFeed` URL**; `sc=`
  ...
  (`DESIGN.md:1277-1279`); **#2 has no such pair in the design and #4 does not supply one** - #4
...
- The stdio arm asserts the attribution marker and **not** the literal `"global"`
  (`DESIGN.md:696-701` - the implementer error this row exists to prevent).
...
- §8 **#16**, read arm: `request_id` present **on the wire result**, under the namespaced key,
  ...
  the output model. `DESIGN.md:1329-1331` is explicit that asserting on the `ToolResult` object
...
- **COLLISION 11 WAS THIS UNIT'S TRAP, AND IT WAS DISARMED BEFORE U5 WAS DISPATCHED - read this
  ...
  **manufactures** a wholly-credentialed file rather than waiting for this unit. **Probe M4 in
  ...
  `DESIGN.md:1242-1247` exists to protect, and `1e67f9c`'s third arm - a genuine orphan outside
...
- §8 **#16, error arm** - the `error_auth_200_body401.json` call in the bullet above returns a
  ...
  (`DESIGN.md:630-636` distinguishes them), so this is a different assertion from the read arm
...
**The result cap is now named:** `JOBVITE_MAX_RESULTS`, default **50** (`DESIGN.md:1569-1572`), and
...
a name (`DESIGN.md:1743`, `:1738`), because what closed was the naming, not the exposure. The plan
...
- §8 **#13**, the concurrent arm: **two invocations driven in parallel**, each forced to retry, each
  log line matched to the invocation that produced it. `DESIGN.md:1310-1312` states that a
...
- The self-throttle honours `JOBVITE_OUTBOUND_RATE_LIMIT`, default **6 requests per minute**
  (`DESIGN.md:1573-1580`). **Say what it is at the point it is used: a conservative guess, not a
...
`DESIGN.md:615` requires the breaker to **evaluate transitions on the call path, not from a
...
the inline breaker of `DESIGN.md:615` is taken with evidence rather than by preference. If it passes,
...
- **Positive control FIRST, because without it three of the arms below are vacuous and two of them
  ...
  Draft 3 applied `DESIGN.md:1367-1368`'s blanket rule to U10 and U14 and skipped U8.
...
- §8 **#19**: fencing, including content that tries to **close its own fence** - the red-team cases
  are merge-gating (`DESIGN.md:752`). `candidate_list_injection.json` is the seed; it is not
...
- Path-keyed, not name-keyed: a test where `title` and `eId` appear at two depths and are decided
  differently - name-keying would collide (`DESIGN.md:745-747`).
...
- **`ResponseCachingMiddleware`, `ErrorHandlingMiddleware`, `ResponseLimitingMiddleware`,
  ...
  excluded for a measured reason (ADR-0004, `DESIGN.md:1175-1200`) and re-adding one is a silent
  regression.
- **Positive control for that absence assertion, and it is what gives the assertion meaning: the
  ...
  is constructed with `include_payloads=False` (C2-I1, `DESIGN.md:1729`). Five absences asserted
...
- §8 **#25**, **both eras**, asserting **the row count did not change** and not the error shape -
  ...
  other.
- **Positive control for #22 and #25, required by `DESIGN.md:1367-1368` and load-bearing here: an
...
**Builds.** The file `DESIGN.md:1513-1519` names as **the advisory-expiry owner** and which does not
...
**The policy this enforces is four ordered steps** (`DESIGN.md:1502-1521`) and step 1 -
...
**Builds.** The two gates of `DESIGN.md:1624-1634`, both of which the design says exceed the
...
with its own test surface, in a repository where **`DESIGN.md:1808` names these two gates as the
mitigation for C8-I1, a Critical row.** A control carrying a Critical, whose own design admits
(`DESIGN.md:1633-1634`) that it *"does nothing about confidential prose pasted into Markdown, which
...
- **The stated ceiling is carried, not quietly dropped:** `DESIGN.md:1632-1634` says this gate stops
...
**Builds.** The README, which `DESIGN.md:1531-1538` **deliberately withholds until now** because a
...
**Verified by.** §8 **#14**'s README arm goes **live** here - `DESIGN.md:1315-1318` requires it
...
**All three carry the blanket positive control of `DESIGN.md:1367-1368`**, and draft 2 stated it for
...
2. **`utils/redaction.py` holds secret redaction (U3) and untrusted-content fencing (U8).**
   `DESIGN.md:1363-1365` names both, and ADR-0010 puts `utils/` at the standard's **95%** because of
...
5. **Tool registration has no stated home and four claimants - and this is the one that breaks
   ...
   (`DESIGN.md:923-927`). U8, U12 and U9 overlap in time by the table above, so if registration
...
5. **The credentialed suite is written as the units land, and never run.** Each tool unit adds its
   ...
   (`DESIGN.md:1244-1248`). Without this the excluded suite rots invisibly for the whole project.
...
2. **Its dependency is unselected (B47) and the selection criterion eliminates the obvious
   candidates.** `DESIGN.md:615` requires transitions to be evaluated **on the call path, not from a
...
3. **A High that just came off the must-mitigate table depends on it.** C5-R1 left the table in
   revision 5 (`DESIGN.md:1864`) on the strength of `request_id_var` plus retry and breaker
...
client.** If nothing passes, the design's sanctioned inline breaker (`DESIGN.md:615`) is taken with
...
|---|---|---|
| ~~Circuit-breaker library vs inline~~ | **Settled by the design in `9d65cc0`, no longer a plan recommendation** | `DESIGN.md:615` sanctions the inline breaker as the fallback where no library evaluates transitions on the call path. Run the survey against the U7 harness; take the inline path if nothing passes. Listed here struck through rather than deleted, because a reader of draft 1 will look for it |
...
- **The threat ids named in §2 are not a coverage map and must not be read as one.** The rows named
  ...
  `DESIGN.md:1845`'s own rule that a count in prose beside the thing it counts is a second source of
  ...
  *"Mitigate before production release"* list at `DESIGN.md:1878-1879` beside C3-I1, C6-D1 and C8-R1,
  ...
  implementation proceeds"* table at `DESIGN.md:1843` is genuinely empty and that is verified - but
...
`.env.example` (`DESIGN.md:1546-1561`). U1 enumerates the full configuration set; U6 and U7 consume
the values and each says at the point of use that **6/min is a conservative guess and not a vendor
figure**, per `:1525-1532`.

**Two things the plan carries forward rather than treating as closed.** `DESIGN.md:1581-1582` says
...
**Answered.** The §7.4 shutdown requirement is now §8 case **#18** (`DESIGN.md:1337-1343`). It
...
§8 case to be named by a §11 row or to cite a B-number or section as its owner. `DESIGN.md:1341`
...
C8-R1, startup configuration logging, remains `unmitigated` (`DESIGN.md:1807`) and on the
...
`DESIGN.md:615` now sanctions an inline breaker in `services/jobvite_client.py` where no library
...
`DESIGN.md:1277-1279` shows the design already knows this shape: #4 is *"positive on purpose"*
...
blanket positive-control rule (`DESIGN.md:1367-1368`) does not list #2 among the refusal-path cases.
