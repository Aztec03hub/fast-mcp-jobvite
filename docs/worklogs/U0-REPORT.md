...
**`DESIGN.md:1762`, threat row C8-I1, says `.env.example` is committed with "empty values".** That
...
| `fastmcp inspect` capability-drift diff | **U1** | `fastmcp inspect --help` confirms `SERVER-SPEC` is required and `server.py` is U1's file. **Standing it up does not execute it** - `DESIGN.md:1493-1496` carries `UNVERIFIED:` and **U0 does not remove that marker** |
...
**Not built: the two commit-time gates of `DESIGN.md:1623-1636`** - pre-commit secret scanning and
...
`DESIGN.md:1280-1286` uses for the audit/PII pair - *"the two are paired so that neither can be
