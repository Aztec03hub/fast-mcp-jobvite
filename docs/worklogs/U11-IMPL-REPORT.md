...
- Emits `--ignore-vuln <id>` pairs on stdout, **derived from the same table rows it validates**. There is no second list of ids anywhere in the file - the two-lists defect `DESIGN.md:1517-1519` names by name.
...
**`pip-audit` is required by `DESIGN.md:1486` and is run by no CI step at all.** Filed as **task #26**; not fixed here.
