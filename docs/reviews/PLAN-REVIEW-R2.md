...
table: every top-level bullet between `DESIGN.md:1184` (*"Required cases"*) and `:1273`
...
|---|---|---|---|
| C3-I1 | `DESIGN.md:1640` | **C2-I1** (`include_payloads` flipped to True) | **`:1654`** |
| C6-D1 | `:1683` | **C5-R1** (retries/breaker not logged) | **`:1697`** |

At `IMPLEMENTATION-PLAN.md:319`, `:512` and `:1095`.

Both are off by exactly -14, and I traced why: **at `9d65cc0`, `DESIGN.md:1640` really was
...
`unmitigated (B15)`, and `DESIGN.md:1789` lists both on the mitigate-before-production-release
list. Only the anchors are wrong.

**Suggested fix (mine, verify before adopting):** replace `DESIGN.md:1640` → `DESIGN.md:1654`
...
`DESIGN.md:407` states the cap once (*"Page cap **500** on v2, **1000** on `/v1/jobFeed`"*), in
...
The blanket rule at `DESIGN.md:1284-1285` is exactly this, and draft 3 applied it correctly to
...
(positive, *"positive on purpose"*) with #5 (absence) - `DESIGN.md:1194-1197`. **#2 has no such
...
`DESIGN.md:1194-1197` and worth raising as a Q6.
