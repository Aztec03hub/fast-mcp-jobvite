"""R6 probe: is arm 1c of probe-breaker-call-path.py a real positive control?

Arm 1c searches the probe's OWN source for SCHEDULING_NAMES and passes when
every name is found.  But SCHEDULING_NAMES is *defined in that same source*,
so each term is present as its own definition.

ARM A  which of the 8 names survive when the SCHEDULING_NAMES tuple literal
       and every docstring/comment are removed?  Those are the only ones the
       probe's executable code actually exercises.
...
print(f"    arm 1c's predicate finds {len(decoy_found)}/8 -> "
      f"{'PASSES (control is tautological)' if len(decoy_found) == 8 else 'fails (control is real)'}")
