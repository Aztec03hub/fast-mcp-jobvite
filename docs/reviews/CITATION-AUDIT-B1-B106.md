...
checklist tick reading "when needed", and `DESIGN.md:856` now presents the file *"at its actual
...
| B55 | *"95% line and 90% branch on critical paths"* | `95% line with 90% branch on` (`DESIGN.md:970`) |
| B58 | *"CI runs --collect-only against the live suite and fails on a collection error"* | `CI runs \`--collect-only\` against it and fails on a collection` (`DESIGN.md:907`) |
