#!/usr/bin/env python3
"""The final verdict per B-number from CONFORMANCE-RESWEEP.md.

CONF-6 finding F-10. This file exists because the resweep's table has
already produced one wrong population, in a report that then drove a
work assignment.

The failure was not carelessness. The table has SEVEN columns -

    B | Requires | Clause (re-verified) | Was | Now | Category |
    Evidence

- and the two that matter sit next to each other. Column 5 (`Now`) is
  the current verdict; column 6 is `Category`, whose vocabulary is
  DEFECT / NOT-YET-BUILT / DEFERRED-WITH-REASON / STILL-OPEN. A parser
  that takes "the last verdict-looking cell" picks up
  DEFERRED-WITH-REASON and reports a DEFERRED *verdict*, which does not
  exist. That is what happened: five rows - B73, B77, B81, B89, B103 -
  moved out of PARTIAL/UNADDRESSED into an invented class, and four of
  the five are the README cluster, so the error made the largest
  remaining gap look smaller than it is.

Six rows (B7, B30, B47, B56, B96, B97) quote markdown tables INSIDE a
cell and carry escaped pipes, so a naive `line.split('|')` gives them
the wrong column count and silently shifts every cell after the escape.
Splitting on unescaped pipes only is not a nicety here; it is the
difference between reading column 5 and reading column 4.

WHAT MAKES THIS DIFFERENT FROM THE PARSER THAT WAS WRONG: it does not
merely produce a count, it asserts its count against the one the
document states about itself in section 1's "The 59 re-walked" table.
Two independent statements of the same number, one from the prose and
one from the rows, have to agree. A count that checks itself is the only
kind that survives being copied forward.

SELECTOR CONTROL, because a wrong zero explains itself: parsing zero
rows, or finding no section 1 table, is a FAILURE and never a pass. A
search at a path that does not exist returns the same clean empty as a
real absence, and this repository has produced that mistake more than
once.

Usage:
    python3 docs/reviews/check-resweep-verdicts.py
    [path/to/CONFORMANCE-RESWEEP.md] python3
    docs/reviews/check-resweep-verdicts.py --controls
...
# Section 1's self-description. The heading is the anchor: section 1
# carries a SECOND table ("Projected across all 106") whose numbers are
# a different population, and reading that one instead would compare 59
# rows against a 106-row claim and call the disagreement a defect.
...
def normalise(cell: str) -> str:
    ...
    `**SATISFIED (design)**` and `SATISFIED` are the same verdict; two
    rows write it differently.
    """
...
def parse_rows(text: str) -> tuple[dict[str, str], list[str]]:
    """{B-number: final verdict}, plus the problems found parsing."""
    ...
    for lineno, line in enumerate(text.splitlines(), 1):
        ...
        if verdict not in VERDICTS:
            problems.append(
                ...
                f"{verdict!r}, which is not a verdict. The Category column is next to "
                f"it and its "
                "vocabulary (DEFERRED-WITH-REASON, STILL-OPEN, NOT-YET-BUILT) reads "
                "like one."
            )
            continue
        if bnum in verdicts:
            problems.append(
                f"{DEFAULT_DOC}:{lineno}: {bnum} appears twice in the table.",
            )
            continue
...
def parse_stated_counts(text: str) -> dict[str, int]:
    """Section 1's counts: the '59 re-walked' table's last column."""
    lines = text.splitlines()
    try:
        start = next(
            i for i, line in enumerate(lines) if line.strip() == COUNTS_HEADING
        )
...
def check(path: pathlib.Path) -> int:
    ...
    # Selector controls. A zero here means the parser matched nothing,
    # which is indistinguishable from a clean document unless it is
    # called out as a failure.
    if not verdicts:
        failures.append(
            "FAIL: parsed zero rows from section 3. Either the table's shape changed "
            "or the row "
            "pattern no longer matches. A green from a parser that read nothing means "
            "nothing."
        )
    if not stated:
        failures.append(
            f"FAIL: found no counts under {COUNTS_HEADING!r}. Without the document's "
            f"own numbers "
            "there is nothing to check the row tally against, and this script degrades "
            "into the unchecked parser it exists to replace."
        )
        ...
        for verdict in VERDICTS:
            ...
            if want is None:
                ...
                failures.append(
                    ...
                    "The prose and the table disagree; one of them has been edited "
                    "alone."
                )
...
# ----------------------------------------------------------------------
# Controls. Every check above is made to fire against a deliberately
...
def _mutate_stated_count(text: str) -> str:
    """Change §1's stated number so the document contradicts itself."""
...
def _mutate_column_shift(text: str) -> str:
    ...
    This is the exact defect that produced the wrong population:
    DEFERRED-WITH-REASON read as a verdict. The parser must refuse it
