...
**C4-D2 is as described.** `DESIGN.md:1602` - *"a model retrying after a timeout, or a human
...
the right precedent already in it: the read-only-key treatment at `DESIGN.md:803-817` /
...
**So the count is now two.** `architecture/caching.md` tripped unnoticed (`DESIGN.md:1869-1873`);
...
**And `DESIGN.md:1874` is now stale in the worst possible direction.** It reads *"`devops/docker.md`
...
- §10 (`DESIGN.md:1270-1300`, and `grep -i "pypi|uvx|wheel|publish"`) describes a Python `>=3.12`
...
### Fix 2 - point C4-D2 at the disposal (`DESIGN.md:1602`, and the Residual Risks row at `:1759`)
...
### Fix 3 - rewrite `DESIGN.md:1874`, in place, not appended
...
### Fix 4 - add one sentence to the freeze procedure at `DESIGN.md:1869`
