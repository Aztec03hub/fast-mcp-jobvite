# MEASURED: the packing probe now fits from three runs, and two of its
# published verdicts do not survive its own spread
...
The probe now prints three fits. Deltas (sharded BEST minus unsharded
BEST) by lane count:

| lanes | MIN fit | MEDIAN fit | MAX fit | |
|---|---:|---:|---:|---|
| 11 | +27.5 | +15.0 | +10.0 | same sign |
| 12 | **+19.0** | **-5.0** | **-12.7** | **SIGN FLIPS** |
| 13 | **+1.5** | **-27.0** | **-37.0** | **SIGN FLIPS** |
| 14 | -20.7 | -43.0 | -54.7 | same sign |
| 15 | -28.0 | -55.0 | -69.0 | same sign |
| 16 | -31.0 | -65.0 | -81.0 | same sign |

- **The 12-lane conclusion does NOT survive.** The single-run probe
  printed `-2.0s WINS`. The median fit says `-5.0s WINS`. The MIN fit
  says `+19.0s` — a loss. **The published 12-lane verdict is an artefact
  of which run was sampled.** This is the cell every repack decision on
  this project rests on, and it is not supportable.
- **The 13-lane conclusion does NOT survive** either, though it is closer
  to safe: it flips only at the extreme MIN envelope (+1.5).
- **The 11-lane loss SURVIVES.** Positive in all three fits (+27.5 /
  +15.0 / +10.0). The single-run text calling it "a proved loss" was
  proved against one run, but the conclusion happens to hold across the
  measured spread. The text's other caveat stands: it remains dependent
  on the fitted shard costs.
- **14, 15 and 16 lanes SURVIVE.** Negative in all three fits, with the
  smallest margin (-20.7s at 14 lanes) still larger than the spread.
- **Every ABSOLUTE published from the single-run fit is void.** The
  12-lane unsharded floor was published as 311.0s; the honest statement
  is 271-346s, median 317s.

## 6. A defect this fix does NOT close, named rather than hidden

`U3_SHARD = 163.3` and `U9_SHARD = 219.5` are still fitted against ONE
run's 304s and 298s, and this probe does not refit them per fit. So the
SHARDED column under the MIN and MAX tables pairs a re-fitted unsharded
input with a median-vintage shard cost. Fixing the unsharded column and
leaving this one is the same defect one column over.

The probe therefore **withholds** the WINS/loses verdict on every fit but
MEDIAN (printing `n/a`), and prints the raw deltas so the sign-flip
analysis above is still visible with that caveat attached. The sign flip
at 12 lanes is *at least* this large; a proper refit could move it
either way. Task #278 separately contests the overhead term.

**Consequence: no repack or shard decision at 12 or 13 lanes is
supportable today.** Closing this needs the shard costs re-fitted per
run, from the same three-run population.

## 7. The guards, watched firing

Every guard below was planted and observed to refuse **before any table
row**, exiting non-zero. The tree was restored after each.

1. **Fewer than `MIN_RUNS` comparable runs.** Observed for real during
   development, when `EXPECT_WORK_SIG` carried a value computed by a
   different hashing route:
...
2. **Duplicate step name.** Planted by removing `Install from the frozen
   lock` from `WRAP` — reproducing the exact defect that motivated the
   guard:
...
4. **Populations differ between accepted runs.** Driven against the
   ...
   `REFUSING: run r2's population differs by ['Y']. A per-step median needs the same steps in every run.`
   exit 1.

The comparability gates are exercised on every ordinary run too:
`0d2c945` is deliberately left in `CANDIDATES` so its rejection is
re-measured each time rather than asserted once in this document.

## 8. Verification

- `ruff check` — All checks passed.
- `ruff format --check` — 1 file already formatted.
- Test-merged onto `main` `d314283`: clean, no conflicts.
- The packing algorithm (`lpt`, `lower_bound`, `descend`, `best`) is
  carried across byte-identical; only the fitting and reporting layers
  changed.
