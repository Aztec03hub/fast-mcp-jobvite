...
UNWIRED_BY_DECISION: dict[str, str] = {
    ...
    "probe-286-u9-coverage.sh": (
        "a PAIRED COMPARISON of one harness against itself, not a gate. It "
        "runs each of U9's 14 amputation rows twice - once against the "
        "coverage-selected set, once against the whole suite - and diffs "
        "the verdicts and the killer sets. Its answer was 14/14 "
        "verdict-preserving and 13/14 killer-set-identical, the exception "
        "being A14, whose four dropped killers use `spawn_marker_server`: "
        "a CHILD PROCESS that `--cov-context` cannot observe, so the map "
        "never records those tests as covering the line. It is UNWIRED "
        "because it takes roughly twice the pole step's runtime to answer "
        "a question about the HARNESS rather than about the tree, and "
        "because a wired step here would re-answer it on every push "
        "forever. Re-run it by hand when U9's selector, its anchors, or "
        "`select-covering-tests.py` change - those are the three inputs "
        "that could move the verdict."
    ),
    "probe-290-u4-coverage.sh": (
        "the same paired comparison as probe-286, for U4, and UNWIRED for "
        "the same reason: it answers a question about the harness, at "
        "roughly twice the row cost, and CI does not need the answer "
        "again once it is recorded. Its result is STRONGER than U9's - "
        "17/17 verdict-preserving AND 17/17 killer-set-identical - and "
        "the blind spot U9 has is structurally impossible here: `grep -n` "
        "over all of tests/test_jobvite_client.py returns zero "
        "subprocess/Popen/spawn sites, and `spawn_marker_server` lives "
        "only in test_boot.py, test_shutdown.py and test_spawn_orphan.py, "
        "none of which is U4's `$SUITE`. The zero FULL-only killers is "
        "the independent confirmation: there was no off-map killer to "
        "lose. It sources scripts/lib/verdict-guard.sh and CALLS the "
        "guard rather than re-implementing the rc discrimination, because "
        "an earlier version hand-rolled the identical `case` and would "
        "have drifted out of the library's reach - which is the defect "
        "this probe exists to look for, one level up."
    ),
