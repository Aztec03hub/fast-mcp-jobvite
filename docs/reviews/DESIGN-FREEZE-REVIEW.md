...
§11's must-mitigate table is empty (`DESIGN.md:1754` reads `*(none)*`), so that half of the freeze
...
`docs/DESIGN.md:757-761` states a supported startup path:
...
... the file an operator copies"* - has no way to express it. `docs/DESIGN.md:1211` builds the §8
...
`docs/DESIGN.md:752-755`, added in the same window, records two more variables that had no name and
...
`docs/DESIGN.md:783-784`: *"the same fail-fast posture §7.3 applies to every required variable."*
`docs/DESIGN.md:888`: *"Fail-fast validates what each *enabled* tool requires, **never the union**"*,
...
`docs/DESIGN.md:1654` (C3-I1) and `:1697` (C6-D1) each contain three `**` markers on one line. The
...
`docs/DESIGN.md:753`: *"That is the same defect as B15, found the same way."* B15 at source
...
- **The new B37 citation resolves.** `docs/DESIGN.md:1266` cites `backend/resilience.md:166-168`
