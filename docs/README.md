...
| Document | What it is |
|---|---|
| [`DESIGN.md`](DESIGN.md) | **The authority, and FROZEN.** The revision counter in its own header is not the record - read the SHA in [`DESIGN-FREEZE.txt`](DESIGN-FREEZE.txt), which is the ONE place it lives and what every brief hands its agent. `reviews/check-design-freeze.py` runs in CI and compares that SHA's blob to the trunk's, because the pointer and the design came apart for a day and no gate asked. Tool surface, module layout, Jobvite client, error contract, testing strategy, threat model. **Only a numbered ADR may change it** - a finding against it is no longer an edit request. Its status block names three risks the freeze carries rather than resolves; read those before trusting it. Everything else either supports it or records how it got that way. |
...
| [`reviews/check-coupling*.py`](reviews/) | The three gates. `check-coupling.py` enforces §11 against §8; the controls harness proves each check can fail; the sweep is subject-free. Run from the repository root. **All three run in CI**, wired the day each went green. Six further gates joined them since: the design-freeze blob check, the no-errexit gate, the row-floor exactness and firing checks, the citation-shape scan and the settings-are-read tripwire. `check-review-coverage.py` is built and deliberately UNWIRED, but no longer for that reason: #151 made it a RATCHET against `reviews/review-coverage-backlog.txt`, so it exits 0 while the recorded set of unreviewed commits matches the measured one. It stays unwired because it belongs on PULL REQUESTS - a merge cannot record its own sha - and wiring it is one `ci.yml` change waiting on #153. |
