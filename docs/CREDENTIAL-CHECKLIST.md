...
|---|---|---|
| 0 | **Two questions about the permission model, asked together because one human answers both.** (a) **Does Jobvite issue a read-only API key**, and if so request one for every deployment running with `JOBVITE_ENABLE_WRITES=false`. (b) **Does Jobvite expose per-user or multi-tenant scoping of any kind** - can one key see a narrower set of records than another? Record both answers here either way | (a) DESIGN.md §7.2 requires a read-only key where writes are disabled, and C5-E1 is a **High** residual. **(b) is what settles §7.2's no-ambient-authority disposal (B107)**, which rests on there being no caller-scoped record set and expires the day Jobvite exposes one - a condition that named no observer until this row did precisely because we cannot enforce it. **No Jobvite endpoint reports a key's own permissions**, so a read-only key and a write-capable key are indistinguishable to us; establishing it by attempting a write is the destructive probe §1.1 forbids. **If the answer is no, the exposure is undiminished** and the Residual Risks entry stands as written - which is the outcome to record plainly, not to leave as an unticked box that reads like an oversight |
